...
Now, before we move on, if you're wondering: "why should removing a taint be considered a valid 'troubleshooting' approach to enabling our pending pods to be scheduled?" then you're not crazy. If it seems a bit counterintuitive, it is. Sure, removing the node taint resolves the issue. But why should we taint the node in the first place then, right?

I've noticed many cases working with teams in the past where they overuse node taints, and it actually makes scheduling pods a bit more confusing. Now, you may also be thinking, "but wait, taints are pretty easy to use, so why is this the case?" And, in my opinion, you would also not be crazy to think this. After all, taints are essentially simple boolean logic. If the node is tainted, and if the pod does not tolerate the taint, then pod_scheduled_to_this_node = False, right? Pretty simple on the surface.

My opinion, however, is that it becomes cumbersome for a team of developers and SREs to work together to make this boolean logic work consistently. Why should we focus on which (likely few) pods should be allowed to tolerate the taint? Now we've created a system of exceptions. I wouldn't want the performance or availability of my application to be subject to a human-influenced (much less programatic) system bound by rigid, true/false logic with potentially numerous edge cases (note that the human influence stems from developers/SREs defining the taints/tolerations). In short, I've found that it can be difficult for teams to remember which nodes are tainted, why they're tainted, and why certain pods should tolerate the taints.

So my proposed "troubleshooting" technique of removing the node taint actually points to the need to rethink our pod scheduling strategy altogether. Perhaps there is a better, more flexible solution for scheduling pods to the appropriate nodes! Let's explore this further in the next section.
